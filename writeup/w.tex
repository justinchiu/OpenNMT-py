...
Recent work has largely leveraged the strong language modeling capabilities of neural models \citep{wiseman2017d2t},
while slowly incorporating the modularity of probabilistic models \citep{wiseman2018template}.
%However, we do not .

The field's focus has shifted from information extraction to generation as the (conditional) language modeling 
capabilities of neural networks have continued to progress.
...
Alignment in translation allows model to properly condition on the source.


% Concerned with jointly learning a generative model and information extraction system.
% Is there an efficiency gain in using the approximate posterior as a separate system rather than finding 
% viterbi best alignments?
...
$p(\bs\mid\bx)$ where $\bs\in\set{0,1}^n$ is a distribution over
...
It may be better to simply use a language model
\begin{description}
\item[Segment Type]
$p(c_t\mid \bc_{<t}, \bx)$
\item[Record LM]
...
\item[Token LM]
$p(y_{t,i,j})\mid\by_{t,i,<j},r_{t,i})$
\end{description}
